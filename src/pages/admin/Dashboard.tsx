...
import { FileText, Users, Briefcase, HelpCircle, PlusCircle, ArrowRight, MessageSquare, UserCheck, Loader2 } from 'lucide-react';
import { Link } from 'react-router-dom';

const Dashboard: React.FC = () => {
  const { services, team, news, faq, clients, inquiries, loading, loadProtectedData } = useData();
  const { user } = useAuth();
                ...
                {news.slice(0, 5).map(item => (
                  <tr key={item.id} className="border-b border-slate-50 hover:bg-blue-50 transition-colors duration-200 cursor-default">
                    ...
                    <td className="py-3 px-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.category === 'Analytics' ? 'bg-purple-100 text-purple-800' :
