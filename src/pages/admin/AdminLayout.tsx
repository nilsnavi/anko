...
import { LayoutDashboard, FileText, Users, HelpCircle, Briefcase, LogOut, Home, UserCheck, User, ImageIcon } from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Медиа', path: '/admin/media', icon: ImageIcon },
  ];
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-xl z-10">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ease-in-out ${isActive
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-all duration-200 hover:scale-[1.02] w-full text-left"
          >
            <Home size={20} />
            <span>На сайт</span>
          </button>
